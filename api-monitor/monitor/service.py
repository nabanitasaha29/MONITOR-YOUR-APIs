...
# # --------------------------------------------------------
# # FAST PARALLEL FUNCTION — handles 1 API call
# # --------------------------------------------------------
# def run_single_api(session, api, group, tokens):
...
#             for tk, tv in dynamic_tokens.items():
#                 headers[key] = headers[key].replace(f"{{{{{tk}}}}}", str(tv))
...
#                         print("\n===============================")
#                         print(f"Extracted token: {token_name} = {extracted}")
#                         print("===============================\n")  
...
#         for api in api_list:
#             futures.append(
#                 executor.submit(run_single_api, session, api, group, tokens)
#             )
...
# -------------------------------------------------------------------
# FIXED: TOKEN API FIRST → ALWAYS RUN SYNCHRONOUSLY
# -------------------------------------------------------------------
def run_single_api(session, api, group, tokens):
    start_time = time.time()
    error_message = None
...
# -------------------------------------------------------------------
# FIXED run_monitor — TOKEN API runs FIRST before spawning threads
# -------------------------------------------------------------------
def run_monitor(selected_group: str | None = None,
                config_path: str | None = None,
                apis: dict | None = None):
    ...
    # STEP 1 → RUN TOKEN API FIRST (sync)
    for group, api_list in apis.items():
        if selected_group and group != selected_group:
            continue
        ...
        for api in api_list:
            if "extract" in api:
                print("\n🔐 Running Token API First:", api["name"])
                token_result = run_single_api(session, api, group, tokens)
                results.append(token_result)

    # STEP 2 → RUN REMAINING APIs IN PARALLEL
    executor = ThreadPoolExecutor(max_workers=20)
    futures = []

    for group, api_list in apis.items():
        for api in api_list:
            if "extract" not in api:  # skip token API
                futures.append(
                    executor.submit(run_single_api, session, api, group, tokens)
                )
